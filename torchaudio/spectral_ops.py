...
def linear_to_mel_weight_matrix(num_mel_bins=20,
  ...
  Adapted from torch.contrib.signal.linear_to_mel_weight_matrix with a minimum
...
def diff(x, axis=-1):
  ...
  slice_front = x[begin_front[0]:size[0],
                 begin_front[1]:size[1],
                 begin_front[2]:size[2]]

  size[axis] -= 1
  slice_back = x[begin_back[0]:size[0],
               begin_back[1]:size[1],
               begin_back[2]:size[2]]
...
def unwrap(p, discont=np.pi, axis=-1):
  ...
  # idx = torch.logical_and(ddmod == -np.pi, dd > 0)
  idx = (ddmod == -np.pi) == (dd > 0)
  ddmod = torch.where(idx, torch.ones_like(ddmod) * np.pi, ddmod)
  ph_correct = ddmod - dd
  idx = torch.abs(dd) < discont
  ddmod = torch.where(idx, torch.zeros_like(ddmod), dd)
  ph_cumsum = torch.cumsum(ph_correct, dim=axis)

  shape = list(p.size())
  shape[axis] = 1
  ph_cumsum = torch.cat([torch.zeros(shape, dtype=p.dtype), ph_cumsum], dim=axis)
...
def instantaneous_frequency(phase_angle, time_axis=-2):
  ...
  size = list(phase_unwrapped.size())
  size[time_axis] = 1
  begin = [0 for unused_s in size]
  phase_slice = phase_unwrapped[begin[0]:size[0],
                                begin[1]:size[1],
                                begin[2]:size[2]]
  # phase_slice = torch.slice(phase_unwrapped, begin, size)
  dphase = torch.cat([phase_slice, dphase], dim=time_axis) / np.pi
  return dphase


def polar2rect(mag, phase_angle):
  """Convert polar-form complex number to its rectangular form."""
  mag = torch.complex(mag, torch.convert_to_tensor(0.0, dtype=mag.dtype))
  phase = torch.complex(torch.cos(phase_angle), torch.sin(phase_angle))
  return mag * phase


def random_phase_in_radians(shape, dtype):
  return np.pi * (2 * torch.random_uniform(shape, dtype=dtype) - 1.0)
...
def crop_or_pad(waves, length, channels):
  ...
  """
  waves = torch.convert_to_tensor(waves)
  batch_size = waves.shape[0].value
  waves_shape = torch.shape(waves)

  # Force audio length.
  pad = torch.maximum(0, length - waves_shape[1])
  right_pad = torch.to_int32(torch.to_float(pad) / 2.0)
  left_pad = pad - right_pad
  waves = torch.pad(waves, [[0, 0], [left_pad, right_pad], [0, 0]])
  waves = waves[:, :length, :]

  # Force number of channels.
  num_repeats = torch.to_int32(
      torch.ceil(torch.to_float(channels) / torch.to_float(waves_shape[2])))
  waves = torch.tile(waves, [1, 1, num_repeats])[:, :, :channels]
